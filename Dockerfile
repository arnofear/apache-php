...
&& docker-php-ext-install opcache pdo_mysql mysqli gd exif zip bz2 ldap bcmath calendar mcrypt xsl xmlrpc soap tidy sockets gettext readline >/dev/null \
&& pecl install apcu \
...
&& docker-php-ext-install sysvmsg >/dev/null \
